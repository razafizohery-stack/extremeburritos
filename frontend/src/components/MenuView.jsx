...
export default function MenuView({ onEdit }) {
  ...
  const fetchMenus = async () => {
    setLoading(true);
    // Fetch menus with their items AND their recipes
    const { data, error } = await supabase
      .from('menus')
      .select(`
        *, 
        menu_items(produits(name)),
        recettes(*, ingredient:ingredient_id(name))
      `)
      .eq('is_active', true);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {menus.map(menu => (
        <div key={menu.id} className="border border-gray-100 rounded-2xl p-6 hover:shadow-lg transition-all shadow-sm flex flex-col">
          ...
          <div className="bg-gray-50 rounded-xl p-4 mb-4 text-sm font-medium text-gray-600">
            <strong>Composants:</strong> {menu.menu_items.map(item => item.produits?.name).join(', ')}
          </div>

          {/* Recette (Ingrédients) */}
          {menu.recettes.length > 0 && (
              <div className="bg-red-50 rounded-xl p-4 mb-4 text-sm font-medium text-red-800">
                <strong>Recette (Stock):</strong>
                <ul className="list-disc ml-4 mt-1">
                    {menu.recettes.map((r, i) => (
                        <li key={i}>{r.ingredient?.name}: {r.quantite_requise} {r.unite_deduction}</li>
                    ))}
                </ul>
              </div>
          )}
